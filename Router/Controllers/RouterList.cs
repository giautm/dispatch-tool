...
namespace GiauTM.CSharp.TikiRouter.Controllers
{
    class RouterList
    {
        ...
        {
            CsvReader csv = new CsvReader(new StreamReader(file), true, '\t');
            ...
            {
                MessageBox.Show("Cấu trúc tệp tin cấu hình không đúng. Cần phải có các trường Router, WardId");
                return false;
            }
            ...
            while (csv.ReadNextRecord())
            {
                ...
                if (router == null)
                {
                    router = new Router { name = routerName, wards = new List<string> { csv[idxWardId] } };
